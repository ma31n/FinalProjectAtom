...
func _ready():
	add_to_group("EnemyPowerUp");
	loadSFX();
	current_enemy=powerupenemy;
...
func movement():
	if($"/root/Global".pause==false):
		...
		move.x=current_enemy.speed*$AnimatedSprite.scale.x;
		move_and_slide(move);

func hit_reg():
	if($Area2DEnemyPowerUp.overlaps_area(player.get_node("Area2DAttack"))):
	...
	elif($Area2DEnemyPowerUp.overlaps_area(player.get_node("Area2DAttack2")) and player.anim=="runningAttack"):
		if(player.enemydestroyable==true and player.anim!="hurt"):
			current_enemy.health-=player.playerdamage[player.anim];
...
func kick_back():
	if(current_enemy.kickback>0 and kicked==true):
		...
		current_enemy.kickback-=kickbackfall*$"/root/Global".speedup;
	else:
...
func _on_Timer_timeout():
	$Area2DEnemyPowerUp.monitorable=true;
	$Area2DEnemyPowerUp.monitoring=true;
	current_enemy.speed=0;
	ogspeed=0;
	$AnimatedSprite.play("attack");
	$"/root/Global".speedup=1.5;
